...
from MSD08_utilities.metrics import dice_coef, batch_iou, mean_iou, iou_score
...
def main():
    ...
    if args.loss == 'BCEWithLogitsLoss':
        ...
        criterion = losses.BCEDiceLoss_3().cuda()

    cudnn.benchmark = True

    # Data loading code
    img_paths = glob('./MSD_data/train_image/*')
    mask_paths = glob('./MSD_data/train_label/*')

    # img_paths = glob(os.path.join(img_paths, '*.npy'))
    # mask_paths = glob(os.path.join(mask_paths, '*.npy'))
    ...
    log = pd.DataFrame(index=[], columns=[
        'epoch', 'lr', 'loss', 'iou', 'dice_2', 'val_loss', 'val_iou', 'val_dice_2'
    ])
    ...
    for epoch in range(args.epochs):
        ...
        print(
            'loss %.4f - iou %.4f - dice_2 %.4f - val_loss %.4f - val_iou %.4f - val_dice_2 %.4f'
            % (train_log['loss'], train_log['iou'], train_log['dice_2'], val_log['loss'],
               val_log['iou'], val_log['dice_2']))
        ...
        tmp = pd.Series([
            ...
            val_log['iou'],
            val_log['dice_2'],
        ], index=['epoch', 'lr', 'loss', 'iou', 'dice_2' ,'val_loss', 'val_iou', 'val_dice_2'])
        ...
        if val_loss < best_loss:
            torch.save(model.state_dict(), 'models/{}/{}/epoch{}-{:.4f}_model.pth'.format(args.name,timestamp,epoch,val_log['dice_2']))
            best_loss = val_loss
